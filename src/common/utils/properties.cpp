...
namespace utils::properties {
	namespace {
		...
		std::filesystem::path get_properties_file() {
			static auto props = [] {
				auto path = std::filesystem::path(std::format("{}_players/properties.json", identification::client::get_client_name()));
	...
	std::filesystem::path get_appdata_path() {
		...
		static auto appdata = std::filesystem::path(path) / identification::client::get_client_name();
		return appdata;
	}

	std::unique_lock<named_mutex> lock() {
		static named_mutex mutex{ std::format("{}-properties-lock", identification::client::get_client_name()) };
