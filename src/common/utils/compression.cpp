...
namespace utils::compression {
	namespace zlib {
		...
		std::string compress(const std::string& data) {
			...
			if (compress2(reinterpret_cast<Bytef*>(result.data()), &length, reinterpret_cast<const Bytef*>(data.data()), static_cast<uLong>(data.size()),
	...
	namespace zip {
		namespace {
			bool add_file(zipFile& zip_file, const std::string& filename, const std::string& data) {
				...
				if (ZIP_OK != zipOpenNewFileInZip64(zip_file, filename.data(), nullptr, nullptr, 0, nullptr, 0, nullptr, Z_DEFLATED, Z_BEST_COMPRESSION, zip_64)) {
					return false;
				}
		...
		namespace {
			...
			private:
				...
				uLong read_file(const voidpf stream, void* buf, const uLong size) {
					...
					const auto end = std::min(this->offset_ + static_cast<std::size_t>(size), file_end);
				...
				static long seek_file_static(const voidpf opaque, const voidpf stream, const ZPOS64_T offset, const int origin) {
