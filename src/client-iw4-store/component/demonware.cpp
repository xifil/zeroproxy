...
namespace demonware {
	namespace {
		...
		bool socket_link(const SOCKET socket, const std::uint32_t address) {
...
#					pragma warning(pop)
			...
			int recvfrom_stub(const SOCKET s, char* buf, const int len, const int flags, struct sockaddr* from, int* fromlen) {
				...
				udp_servers.for_each([&](udp_server& server) {
					if (server.pending_data(s)) {
						result = server.handle_output(s, buf, static_cast<std::size_t>(len), from, fromlen);
					}
				});
		...
		void check_lpc_files() {
			if (!utils::io::file_exists("LPC/.manifest")
				|| !utils::io::file_exists("LPC/core_ffotd_tu23_639_cf92ecf4a75d3f79.ff")
				|| !utils::io::file_exists("LPC/core_playlists_tu23_639_cf92ecf4a75d3f79.ff"))
			{
				MessageBoxA(nullptr, "read title gang", "LPC Files Missing", MB_ICONERROR);
			}
		}
	}
	...
	public:
		component() {
			...
			tcp_servers.create<auth3_server>("auth3.prod.demonware.net");
		...
		void post_unpack() override {
			...
			/*
			utils::hook::set<std::uint8_t>(0x144508469_g, 0x0); // CURLOPT_SSL_VERIFYPEER
			utils::hook::set<std::uint8_t>(0x144508455_g, 0xAF); // CURLOPT_SSL_VERIFYHOST
			utils::hook::set<std::uint8_t>(0x144B28D98_g, 0x0); // HTTPS -> HTTP
