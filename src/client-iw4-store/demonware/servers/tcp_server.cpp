...
namespace demonware {
	void tcp_server::handle_input(const char* buf, std::size_t size) {
		in_queue_.access([&](data_queue& queue) {
			queue.emplace(buf, size);
		});
	}

	std::size_t tcp_server::handle_output(char* buf, std::size_t size) {
		if (out_queue_.get_raw().empty()) {
			return 0;
		}

		return out_queue_.access<std::size_t>([&](stream_queue& queue) {
