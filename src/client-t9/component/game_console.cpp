...
namespace game_console {
	/*namespace {
		...
		float color_title[4] = { 1.f, 0.89f, 0.43f, 1.f };
		...
		void print_internal(const std::string& data) {
			con.output.access([&](output_queue& output) {
				if (con.visible_line_count > 0 && con.display_line_offset == (output.size() - con.visible_line_count)) {
		...
		void check_resize() {
			con.screen_min[0] = 6.f;
			con.screen_min[1] = 6.f;
			con.screen_max[0] = game::ScrPlace_GetViewPlacement()->realViewportSize[0] - 6.f;
			con.screen_max[1] = game::ScrPlace_GetViewPlacement()->realViewportSize[1] - 6.f;

			if (console_font) {
				con.font_height = console_font->pixelHeight;
				con.visible_line_count = static_cast<int>((con.screen_max[1] - con.screen_min[1] - (con.font_height * 2)) - 24.f) / con.font_height;
				con.visible_pixel_width = static_cast<int>(((con.screen_max[0] - con.screen_min[0]) - 10.f) - 18.f);
			}
			else {
				con.font_height = 0;
		...
		void draw_box(const float x, const float y, const float w, const float h, float* color) {
			t9::vec4_t dark_color{ 0.f, 0.f, 0.f, 0.f };

			dark_color.v[0] = color[0] * 0.5f;
			dark_color.v[1] = color[1] * 0.5f;
			dark_color.v[2] = color[2] * 0.5f;
			dark_color.v[3] = color[3];

			game::R_AddCmdDrawStretchPic(x, y, w, h, 0.f, 0.f, 0.f, 0.f, color, material_white);
			game::R_AddCmdDrawStretchPic(x, y, 2.f, h, 0.f, 0.f, 0.f, 0.f, dark_color, material_white);
			game::R_AddCmdDrawStretchPic((x + w) - 2.f, y, 2.f, h, 0.f, 0.f, 0.f, 0.f, dark_color, material_white);
			game::R_AddCmdDrawStretchPic(x, y, w, 2.f, 0.f, 0.f, 0.f, 0.f, dark_color, material_white);
			game::R_AddCmdDrawStretchPic(x, (y + h) - 2.f, w, 2.f, 0.f, 0.f, 0.f, 0.f, dark_color, material_white);
		}

		void draw_input_box(const int lines, float* color) {
			draw_box(con.globals.x - 6.f, con.globals.y - 6.f, (con.screen_max[0] - con.screen_min[0]) - ((con.globals.x - 6.f) - con.screen_min[0]),
				(lines * con.globals.font_height) + 12.f, color);
		}

		void draw_input_text_and_over(const char* str, float* color) {
			game::R_AddCmdDrawText(str, 0x7FFFFFFF, console_font, con.globals.x, con.globals.y + con.globals.font_height, 1.f, 1.f, 0.f, color, 0);
			con.globals.x = game::R_TextWidth(str, 0, console_font) + con.globals.x + 6.f;
		}

		float draw_hint_box(const int lines, float* color, [[maybe_unused]] float offset_x = 0.f, [[maybe_unused]] float offset_y = 0.f) {
			const auto _h = lines * con.globals.font_height + 12.f;
			const auto _y = con.globals.y - 3.f + con.globals.font_height + 12.f + offset_y;
			const auto _w = (con.screen_max[0] - con.screen_min[0]) - ((con.globals.x - 6.f) - con.screen_min[0]);

			draw_box(con.globals.x - 6.f, _y, _w, _h, color);
			return _h;
		}

		void draw_hint_text(const int line, const char* text, float* color, const float offset_x = 0.f, const float offset_y = 0.f) {
			const auto _y = con.globals.font_height + con.globals.y + (con.globals.font_height * (line + 1)) + 15.f + offset_y;

			game::R_AddCmdDrawText(text, 0x7FFFFFFF, console_font, con.globals.x + offset_x, _y, 1.f, 1.f, 0.f, color, 0);
		}
		...
		void draw_input() {
			...
			con.globals.x = con.screen_min[0] + 6.f;
			con.globals.y = con.screen_min[1] + 6.f;
			con.globals.left_x = con.screen_min[0] + 6.f;

			draw_input_box(1, dvars::con_inputBoxColor->current.vector);

			draw_input_text_and_over("t9-mod-zr-proxy//console-test: " VERSION ">", color_title);

			con.globals.left_x = con.globals.x;
			con.globals.auto_complete_choice[0] = 0;

			game::R_AddCmdDrawTextWithCursor(con.buffer, 0x7FFFFFFF, console_font, 18, con.globals.x, con.globals.y + con.globals.font_height,
				1.f, 1.f, 0, color_white, 0, con.cursor, '|');
			...
			if (matches.size() > 24) {
				...
				draw_hint_text(0, utils::string::va("%i matches (too many to show here, press shift+tilde to open full console, press tab to print them all)",
					matches.size()), dvars::con_inputDvarMatchColor->current.vector);
			}
				...
				draw_hint_text(0, matches[0].name.data(), dvar ? dvars::con_inputDvarMatchColor->current.vector : dvars::con_inputCmdMatchColor->current.vector);

				if (dvar) {
					const auto offset = (con.screen_max[0] - con.globals.x) / 4.f;

					draw_hint_text(0, game::Dvar_ValueToString(dvar, true, dvar->current), dvars::con_inputDvarValueColor->current.vector, offset);
					draw_hint_text(1, "  default", dvars::con_inputDvarInactiveValueColor->current.vector);
					draw_hint_text(1, game::Dvar_ValueToString(dvar, true, dvar->reset), dvars::con_inputDvarInactiveValueColor->current.vector, offset);

					const auto offset_y = height + 2.f; // was 3
					const auto line_count_ = dvar->type == game::dvar_type::enumeration ? dvar->domain.enumeration.stringCount + 1 : 2;
					...
					draw_hint_text(1, dvars::dvar_get_domain(dvar->type, dvar->domain).data(), dvars::con_inputCmdMatchColor->current.vector, 0, offset_y);
				}
		...
		void draw_output_scrollbar(const float x, float y, const float width, const float height, output_queue& output) {
			const auto _x = (x + width) - 10.f;
			draw_box(_x, y, 10.f, height, dvars::con_outputBarColor->current.vector);
			...
			if (output.size() > con.visible_line_count) {
				...
				y += remainingSpace * percentageAbove;
			}

			draw_box(_x, y, 10.f, _height, dvars::con_outputSliderColor->current.vector);
		}

		void draw_output_text(const float x, float y, output_queue& output) {
			const auto offset = output.size() >= con.visible_line_count ? 0.f : (con.font_height * (con.visible_line_count - output.size()));
			...
			for (auto i = 0; i < con.visible_line_count; i++) {
				...
				game::R_AddCmdDrawText(output.at(index).data(), 0x7FFF, console_font, x, y + offset, 1.f, 1.f, 0.f, color_white, 0);
			}
		}

		void draw_output_window() {
			con.output.access([](output_queue& output) {
				draw_box(con.screen_min[0], con.screen_min[1] + 32.f, con.screen_max[0] - con.screen_min[0], (con.screen_max[1] - con.screen_min[1]) - 32.f,
					dvars::con_outputWindowColor->current.vector);

				const auto x = con.screen_min[0] + 6.f;
				const auto y = (con.screen_min[1] + 32.f) + 6.f;
				const auto width = (con.screen_max[0] - con.screen_min[0]) - 12.f;
				const auto height = ((con.screen_max[1] - con.screen_min[1]) - 32.f) - 12.f;
	...
	bool console_char_event(const int local_client_num, const int key) {
		if (key == game::keyNum_t::K_GRAVE || key == game::keyNum_t::K_TILDE || key == '|' || key == '\\') {
			return false;
		}
		...
		if (*game::keyCatchers & 1) {
			if (down) {
				...
				if (key == game::keyNum_t::K_MWHEELUP || key == game::keyNum_t::K_PGUP) {
					...
					con.output.access([](output_queue& output) {
						if (output.size() > con.visible_line_count && con.display_line_offset < (output.size() - con.visible_line_count)) {
							con.display_line_offset++;
						}
					});
				}
	...
	public:
		void post_unpack() override {
			...
			con.globals.x = 0.f;
			con.globals.y = 0.f;
			con.globals.left_x = 0.f;
			con.globals.font_height = 0.f;
			con.globals.may_auto_complete = false;
			...
			// add our dvars
			dvars::con_inputBoxColor = dvars::register_vec4("con_inputBoxColor", 0.f, 0.f, 0.f, 0.7f, 0.f, 1.f,
				console_flag,
				"color of console input box");
			dvars::con_inputHintBoxColor = dvars::register_vec4("con_inputHintBoxColor", 0.f, 0.f, 0.f, 0.7f,
				0.f, 1.f,
				console_flag, "color of console input hint box");
			dvars::con_outputBarColor = dvars::register_vec4("con_outputBarColor", 0.5f, 0.5f, 0.5f, 0.6f, 0.f,
				1.f, console_flag,
				"color of console output bar");
			dvars::con_outputSliderColor = dvars::register_vec4("con_outputSliderColor", 1.f, 0.89f, 0.43f, 1.f,
				0.f, 1.f,
				console_flag, "color of console output slider");
			dvars::con_outputWindowColor = dvars::register_vec4("con_outputWindowColor", 0.f, 0.f, 0.f, 0.5f,
				0.f,
				1.f, console_flag, "color of console output window");
			dvars::con_inputDvarMatchColor = dvars::register_vec4("con_inputDvarMatchColor", 1.f, 1.f, 0.8f, 1.f,
				0.f,
				1.f, console_flag, "color of console matched dvar");
			dvars::con_inputDvarValueColor = dvars::register_vec4("con_inputDvarValueColor", 1.f, 1.f, 1.f, 1.f,
				0.f,
				1.f, console_flag, "color of console matched dvar value");
			dvars::con_inputDvarInactiveValueColor = dvars::register_vec4(
				"con_inputDvarInactiveValueColor", 0.8f, 0.8f,
				0.8f, 1.f, 0.f, 1.f, console_flag,
				"color of console inactive dvar value");
			dvars::con_inputCmdMatchColor = dvars::register_vec4("con_inputCmdMatchColor", 0.80f, 0.80f, 1.f, 1.f,
				0.f,
				1.f, console_flag, "color of console matched command");
		}
	};*/
}
