#pragma once
#include "common.hpp"

namespace demonware {
	class bit_buffer final {
	public:
		bit_buffer() = default;

		explicit bit_buffer(std::string buffer)
			: buffer_(std::move(buffer))
		{}

		bool read_bytes(std::uint32_t bytes, std::uint8_t* output);
		bool read_bool(bool* output);
		bool read_uint32(std::uint32_t* output);
		bool read_data_type(char expected);

		bool write_bytes(std::uint32_t bytes, const char* data);
		bool write_bytes(std::uint32_t bytes, const std::uint8_t* data);
		bool write_bool(bool data);
		bool write_int32(int data);
		bool write_uint32(std::uint32_t data);
		bool write_data_type(char data);

		bool read(std::uint32_t bits, void* output);
		bool write(std::uint32_t bits, const void* data);

		void set_use_data_types(bool use_data_types);

		std::uint32_t size() const;

		std::string& get_buffer();

	private:
		std::string buffer_{};
		std::uint32_t current_bit_ = 0;
		bool use_data_types_ = true;
	};
}
